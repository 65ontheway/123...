...
// Side preferences are settings, not roster content — this never involves
// the model at all, same as the rest of this file. Only the roles present
// in the request body change; anything omitted keeps its current saved
// value (see soccerLineup.applyLineupSettings).
router.put('/roster/settings', requireAuth, async (req, res) => {
  const { defender, midfielder, forward } = req.body || {};
  try {
    const sidePreferences = await soccerLineup.withRosterLock(req.session.username, async () => {
      let roster;
      try {
        roster = soccerLineup.loadRoster(req.session.username);
      } catch {
      ...
      return rosterForEdit.sidePreferences;
    });
    res.json({ ok: true, sidePreferences });
  } catch (err) {

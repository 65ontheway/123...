...
function validateOneToolCall(call, tools, ctx) {
  ...
  for (const map of [args.resting, args.pinned]) if (map && Object.keys(map).some(q => !/^[1-4]$/.test(q))) return { valid: false, reason: 'a quarter key was not 1-4' };
  if (Object.values(args.pinned || {}).some(pins => Object.values(pins).some(position => !require('./soccerFormations').normalizePositionToken(position)))) return { valid: false, reason: 'a pinned position was not recognized' };

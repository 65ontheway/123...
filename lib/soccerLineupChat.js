...
async function handleSoccerLineupChat(res, { upstreamMessages, selectedModel, maxTokens, apiKey, username, ownerId, gameId, appUrl, session }) {
  ...
  if (canonical && toolCalls.some(call => !permitsTool(canonical, call.function.name))) return sendLocalReply(res, 'The AI proposed an action you did not request. Nothing was changed.');
  ...
  const consequential = !canonical || toolCalls.some(call => call.function.name !== 'set_game_lineup');
  if (consequential) {

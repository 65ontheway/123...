...
async function handleSoccerLineupChat(res, { upstreamMessages, selectedModel, maxTokens, apiKey, username, ownerId, gameId, appUrl, session }) {
  ...
  // Only a call that actually changes SAVED state needs a confirm step —
  // generate_lineup_alternative just proposes another draft option (it
  // never overwrites anything saved, and asking twice for "another option"
  // is exactly the kind of back-and-forth this agent should support
  // without friction), so it always executes immediately, whether it came
  // from a canonical command or free-form conversation.
  const SAVED_STATE_TOOLS = new Set(['set_game_lineup', 'manage_roster', 'update_lineup_settings', 'finalize_lineup']);
  const consequential = validToolCalls.some(call => SAVED_STATE_TOOLS.has(call.function.name));
  if (consequential) {

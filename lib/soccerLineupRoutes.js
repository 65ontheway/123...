...
// A one-off constraint (resting/pinned/sideOverrides/ignoreSidePreferences)
// for THIS regeneration only — never persisted into the game's frozen
// inputs, so a plain "generate another option" call right after still
// starts from the original constraints. Reuses normalizeCreateOptions'
// validation so the same input shape/rules apply as game creation.
function normalizeAlternativeOverrides(body = {}) {
  const normalized = normalizeCreateOptions(body);
  const overrides = {};
  if (normalized.resting) overrides.resting = normalized.resting;
  if (normalized.pinned) overrides.pinned = normalized.pinned;
  if (normalized.sideOverrides) overrides.sideOverrides = normalized.sideOverrides;
  if (normalized.ignoreSidePreferences != null) overrides.ignoreSidePreferences = normalized.ignoreSidePreferences;
  return overrides;
}
...
router.post('/games/:gameId/alternative', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const constraintOverrides = normalizeAlternativeOverrides(req.body);
    const { distinctFromPrevious } = await history.withLineupLock(req.session.username, () =>
      history.addAlternative(req.session.username, req.params.gameId, { rotationBoost, constraintOverrides })
    );
    const game = history.getGame(req.session.username, req.params.gameId, roster);

...
// with. The same holds for a slot Pass 3 filled under the coach's weak-
// player placement preference (left wing/right back/striker, see
// soccerScheduling.js's WEAK_PREFERENCE_POSITIONS) — that's every bit as
// deliberate a placement as an exact pin, so it's excluded from the swap
// the same way. Mutates `lineup` in place; changes nothing else
// (selection, quarters-played bookkeeping) — playing time is decided
...
function applySideAssignment(formationName, lineup, effectivePreferences) {
  ...
  for (const group of getSidePairGroups(formationName)) {
    ...
    if (leftSlot.weakPreference || rightSlot.weakPreference) continue;

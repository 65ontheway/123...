...
function hydrateResult(storedResult, playersById) {
  ...
  return {
    ...
    quarters: storedResult.quarters.map((q) => ({
      ...
      resting: (q.resting || []).map((id) => resolve(id)).filter(Boolean),
    })),
...
function buildNewGame(existingGamesData, roster, options = {}) {
  ...
  if (!require('./validation').validDate(date)) throw new LineupHistoryError('INVALID_DATE', 'Use a valid calendar date.');
...
// Merges a one-off constraint override (resting/pinned/sideOverrides/
// ignoreSidePreferences) on top of a game's FROZEN constraints for a
// SINGLE "generate another option" attempt — never mutates or persists
// into frozenInputs itself, so every other draft/regeneration for this
// game still starts from the original constraints (same "one-off, not
// sticky" treatment addAlternative already gives rotationBoost). A
// per-quarter resting/pinned entry is combined with, not replacing, that
// quarter's base entries; a pin conflict on the exact same player+quarter
// lets the override win, since it's the newer, more specific instruction.
function mergeConstraintOverrides(base, overrides) {
  if (!overrides || Object.keys(overrides).length === 0) return base;
  const merged = { ...base };

  if (overrides.pinned) {
    merged.pinned = { ...(base.pinned || {}) };
    for (const [quarter, byPlayer] of Object.entries(overrides.pinned)) {
      merged.pinned[quarter] = { ...(merged.pinned[quarter] || {}), ...byPlayer };
    }
  }
  if (overrides.resting) {
    merged.resting = { ...(base.resting || {}) };
    for (const [quarter, ids] of Object.entries(overrides.resting)) {
      merged.resting[quarter] = [...new Set([...(merged.resting[quarter] || []), ...(ids || [])])];
    }
  }
  if (overrides.sideOverrides) {
    merged.sideOverrides = { ...(base.sideOverrides || {}), ...overrides.sideOverrides };
  }
  if (overrides.ignoreSidePreferences != null) {
    merged.ignoreSidePreferences = overrides.ignoreSidePreferences;
  }
  return merged;
}

// Public: "generate another option" — a new seed, the SAME frozen roster
// snapshot/constraints/settings/history this game started with, optionally
// with a one-off `constraintOverrides` (resting/pinned/sideOverrides/
// ignoreSidePreferences) layered on top for this attempt only — e.g. the
// coach asking for another option AND "make sure Player_2 plays defense
// this time." Tries up to MAX_ALTERNATIVE_ATTEMPTS distinct seeds looking
// for a result that isn't identical to any option already generated for
// this game; if none turns up, the last attempt is kept and
// `distinctFromPrevious: false` is recorded so the caller can explain
// rather than overpromise. Caller must already hold withLineupLock for
// this username.
function addAlternative(username, gameId, options = {}) {
  const { rotationBoost = 1, constraintOverrides = null } = options;
  const data = loadGames(username);
  ...
  const effectiveConstraints = mergeConstraintOverrides(game.frozenInputs.constraints, constraintOverrides);
  ...
  game.drafts[draftId] = {
    ...
    ...(constraintOverrides && Object.keys(constraintOverrides).length > 0 ? { constraintOverrides } : {}),

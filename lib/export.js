...
function validateExportInput(input) {
  if (!input || typeof input !== 'object' || Array.isArray(input) || Object.keys(input).some(key => !['format', 'filename', 'title', 'content', 'markdown', 'rows', 'headers', 'sheets'].includes(key))) throw new ExportValidationError('Invalid export fields.');
  ...
  if ((title || '').length > 500 || filename.length > 80 || JSON.stringify(input).length > 1000000) throw new ExportValidationError('Export is too large.');
  ...
  if (TABLE_FORMATS.has(format)) {
    ...
    if (format === 'csv' && normalizedSheets.length > 1) throw new ExportValidationError('CSV supports one sheet; use XLSX for multiple sheets.');
    for (const sheet of normalizedSheets) {
      const sheetRows = sheet?.rows;
      if (!Array.isArray(sheetRows)) throw new ExportValidationError('Each sheet needs an array of rows.');
      if (sheet.name != null && (typeof sheet.name !== 'string' || sheet.name.length > 31)) throw new ExportValidationError('Invalid sheet name.');
      for (const row of [...(sheetRows || []), ...(sheet.headers ? [sheet.headers] : [])]) {
        if (!Array.isArray(row) || row.length > 50) throw new ExportValidationError('Each row must have at most 50 cells.');
        for (const cell of row) if (!(cell === null || typeof cell === 'boolean' || typeof cell === 'string' && cell.length <= 4000 || typeof cell === 'number' && Number.isFinite(cell))) throw new ExportValidationError('Cells must be bounded text, finite numbers, booleans or null; formulas are unsupported.');
      }
  ...
  return {
    ...
    content: content || '',
    markdown: markdown || '',
    rows: format === 'csv' ? normalizedSheets[0].rows : Array.isArray(rows) ? rows : [],
    headers: format === 'csv' ? normalizedSheets[0].headers || [] : Array.isArray(headers) ? headers : [],
...
function generatePdfBuffer({ title, content, markdown }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.on('data', chunk => {
      bytes += chunk.length;
      if (bytes > 10 * 1024 * 1024) { doc.destroy(); reject(new ExportValidationError('PDF exceeds 10 MiB.')); }
...
async function generateExport(rawInput) {
  if (generating >= 2) throw new ExportValidationError('Two exports are already being generated. Try again shortly.');
  generating++;
  try { return await buildExport(rawInput); } finally { generating--; }
}
async function buildExport(rawInput) {
  ...
  if (buffer.length > 10 * 1024 * 1024) throw new ExportValidationError('Generated file exceeds 10 MiB.');

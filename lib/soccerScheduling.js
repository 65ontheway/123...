  ...
  for (const q of QUARTERS) {
    ...
    for (const slot of fillOrder) {
      ...
      if (remaining.length === 0) {
        // Every remaining slot in this quarter is equally unfillable — the
        // available pool never grows back within a quarter, so one warning
        // for the whole quarter is correct; repeating it per slot just
        // spammed the same line once per still-open position.
    ...
    // Kept separate from `bench`: a resting player was never a candidate
    // for a slot this quarter at all (excluded from `available` above),
    // so folding them into "bench" would silently overstate how many
    // players the fill algorithm actually had to choose from. Both a
    // formatter and a coach need to see them distinctly, or a resting
    // request reads as the bench count simply being wrong.
    quarters.push({ quarter: q, lineup, bench, resting: restingPlayers });
  }
...
function formatGameLineupResult({ formation, quarters, quartersPlayedSummary, sidePreferencesApplied, warnings }) {
  ...
  for (const { quarter, lineup, bench, resting } of quarters) {
    ...
    if (resting && resting.length > 0) lines.push(`Resting: ${resting.map((p) => p.name).join(', ')}`);
  }

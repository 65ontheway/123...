...
function initRosterStorage() {
  ...
  return migrateLegacyRosters(dir, process.env.RAYGPT_LEGACY_ROSTER_DIR);
}
...
function isValidRosterShape(parsed) {
  return !!parsed && typeof parsed === 'object' && Array.isArray(parsed.players) && parsed.players.length <= 100 && parsed.players.every(player => player && typeof player.name === 'string' && player.name.length <= 100 && player.skills && ['offense', 'defense', 'goalie'].every(key => Number.isInteger(player.skills[key]) && player.skills[key] >= 1 && player.skills[key] <= 5));
}
...
function loadRosterResult(username) {
  ...
  if (cached && cached.mtimeMs === stat.mtimeMs) {
    return { status: 'ok', roster: structuredClone(cached.roster) };
  }
  ...
  rosterCache.set(username, { roster: structuredClone(parsed), mtimeMs: stat.mtimeMs });
  return { status: 'ok', roster: parsed };
}
...
function saveRoster(username, roster) {
  ...
  rosterCache.set(username, { roster: structuredClone(roster), mtimeMs: fs.statSync(file).mtimeMs });
}
...
function clampRating(value) {
  if (typeof value !== 'number' || !Number.isInteger(value) || value < 1 || value > 5) throw new RosterValidationError('Ratings must be whole numbers from 1 to 5.');
  return value;
}
...
function addPlayerDirect(roster, { name, offense, defense, goalie } = {}) {
  ...
  if (roster.players.length >= 100) throw new RosterValidationError('The roster is limited to 100 players.');
  const player = {
...
module.exports = {
  ...
  applyFormationSetting: formations.applyFormationSetting,

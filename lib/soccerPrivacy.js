// Local name/label translation helpers. These do not anonymize arbitrary text.
// soccerRequest.js gates complete commands before any model request; history,
// standing facts, attachments and saved tool results never pass through it.
const soccerLineup = require('./soccerLineup');
const { isValidDateString } = require('./soccerLineupTools');
...
// local execution uses the same context object).
function buildAnonymizationContext(roster) {
  ...
  roster.players.forEach((player, index) => {
    ...
    const key = player.name.normalize('NFC').trim().toLowerCase();
  ...
  const namesForMatching = [...new Set(roster.players.map((p) => p.name.normalize('NFC').trim()))]
    .filter(Boolean)
    .sort((a, b) => b.length - a.length);
  const matchRegex = namesForMatching.length
    ? new RegExp(`(?<![\\p{L}\\p{N}_])(${namesForMatching.map(escapeRegex).join('|')})(?![\\p{L}\\p{N}_])`, 'giu')
    : null;
...
// A best-effort local gate: if a message looks like it might be trying to
// introduce a NEW player by name, don't send it to the model at all — a
// brand-new name has no existing roster entry to map to a label, so
// there's no way to scrub it. Still biased toward false positives
// (redirecting a message that wasn't really an add-attempt is just mildly
// annoying) over false negatives (a real name reaching the model), but
// narrowed from a bare "add" to phrasing that actually names someone
// joining the roster — "add"/"sign up"/"enroll"/"register" alone are
// common in ordinary strategy talk ("add more defensive cover", "sign up
// for a scrimmage") and shouldn't bounce a normal question to the roster
// panel.
//
// Two independent signals catch a real add attempt: an explicit
// destination ("add ___ to the team/roster/squad") that works regardless
// of capitalization, and — deliberately case-SENSITIVE, unlike the rest
// of this file's matching — "add" directly followed by a capitalized
// word, since a coach naturally phrases introducing someone by name
// ("Add Jordan...", "add Fixture Charlie, she's...") rather than an
// ordinary lowercase word ("add more defensive cover").
const ADD_TARGET_PHRASE_REGEX =
  /\badd\s+(?:\S+\s+){0,4}?(?:to\s+(?:the\s+|my\s+)?(?:team|roster|squad)\b|as\s+an?\s+(?:new\s+)?(?:player|teammate)\b)/i;
const ADD_CAPITALIZED_NAME_REGEX = /\b[Aa]dd\s+[A-Z][\w'-]*/;
const ADD_KEYWORD_REGEX =
  /\badd\s+(?:a\s+)?new\s+(?:player|kid|girl|boy|teammate)\b|\bsign(?:ing)?\s*up\s+(?:a\s+)?(?:new\s+)?(?:player|kid|girl|boy|teammate)\b|\benroll(?:ing)?\s+(?:a\s+)?(?:new\s+)?(?:player|kid|girl|boy|teammate)\b|\bregister(?:ing)?\s+(?:a\s+)?(?:new\s+)?(?:player|kid|girl|boy|teammate)\b|\bnew\s+(?:player|kid|girl|boy|teammate)\b/i;

function looksLikeAddRequest(text) {
  if (!text) return false;
  return ADD_TARGET_PHRASE_REGEX.test(text) || ADD_CAPITALIZED_NAME_REGEX.test(text) || ADD_KEYWORD_REGEX.test(text);
}
...
function translateSchedulingArgsToIds(args, ctx) {
  ...
  if (args.date != null) {
    if (isValidDateString(args.date)) {
      translated.date = args.date;
    } else {
      warnings.push(`"${args.date}" isn't a valid date (use YYYY-MM-DD) — using today's date instead.`);
    }
  }
...
// Same job as formatGameLineupResultAnonymized, for the STORED (id-only)
// result shape soccerLineupHistory.js persists — see that file's
// serializeResult(). Simpler than the live-result version since storage is
// already id-based (no `.player.id` indirection, no name to ever touch),
// which is exactly why the chat flow's draft/alternative/finalize paths
// use this one: there's no intermediate object anywhere that could carry a
// real name into AI-bound text, even by accident.
function formatStoredGameResultAnonymized(storedResult, ctx, extraWarnings = []) {
  const labelFor = (id) => (id ? ctx.labelByPlayerId.get(id) || '(unknown)' : '(unfilled)');
  const lines = [`Formation: ${storedResult.formation}`, '', 'Side preferences applied this lineup:'];
  for (const entry of storedResult.sidePreferencesApplied) {
    const valueLabel = entry.value === 'left' ? 'Left' : entry.value === 'right' ? 'Right' : 'No preference';
    const sourceLabel = entry.source === 'temporary' ? 'this lineup only' : 'saved default';
    lines.push(`- ${entry.role[0].toUpperCase()}${entry.role.slice(1)} → ${valueLabel} (${sourceLabel})`);
  }
  for (const { quarter, lineup, bench, resting } of storedResult.quarters) {
    lines.push('', `Quarter ${quarter}:`);
    for (const slot of lineup) {
      const posLabel = soccerLineup.POSITION_CATALOG[slot.position]?.label || slot.position;
      lines.push(`${posLabel}: ${labelFor(slot.playerId)}`);
    }
    if (bench.length > 0) lines.push(`Bench: ${bench.map(labelFor).join(', ')}`);
    if (resting && resting.length > 0) lines.push(`Resting: ${resting.map(labelFor).join(', ')}`);
  }
  lines.push('', 'Quarters played this game:');
  for (const entry of storedResult.quartersPlayedSummary) {
    lines.push(`${labelFor(entry.id)}: ${entry.quartersPlayed}`);
  }
  const allWarnings = [...extraWarnings, ...storedResult.warnings];
  if (allWarnings.length > 0) {
    lines.push('', 'Warnings:');
    for (const warning of allWarnings) {
      lines.push(`- ${scrubText(warning, ctx).text}`);
...
module.exports = {
  ...
  formatStoredGameResultAnonymized,
  deanonymize,
};

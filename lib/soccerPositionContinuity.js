...
// suitability variety): when a player plays the SAME broad role (defender,
// or midfielder) in two CONSECUTIVE quarters, keep them at the exact same
// position for the second quarter rather than letting the normal fill/
// side-assignment logic shuffle them to a different spot within that role
// — e.g. left_back both quarters, not left_back then right_back; left_wing
// both quarters, not left_wing then center_mid. The coach explicitly wants
// this to win even when it produces a very slightly less optimal lineup:
// staying comfortable at one spot matters more than a marginal skill gain
// from reshuffling. Deliberately scoped to a role staying the SAME across
// the two quarters — a player whose role itself changes between quarters
// (e.g. defender last quarter, midfielder this quarter) is a rotation
...
// already-selected group of continuing defenders/midfielders, which exact
// slot each one lands in — so it composes the same way side-assignment
// does (a pure re-arrangement after selection, not a selection decision).
// Runs AFTER side-assignment and simply overrides its placement for any
// continuing player who has a home to return to: continuity is the
// coach's explicit ask, so it wins over the side-preference heuristic for
// those players specifically, while every other slot (new subs off the
// bench, a role change, a player with no defender/midfielder history from
// last quarter) keeps whatever side-assignment already decided.
//
...
// else's continuity).
const CONTINUITY_ROLES = new Set(['defender', 'midfielder']);

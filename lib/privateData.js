...
function resolvePrivateDataDir() {
  ...
  const suffix = [];
  while (!fs.existsSync(ancestor)) { suffix.unshift(path.basename(ancestor)); const parent = path.dirname(ancestor); if (parent === ancestor) break; ancestor = parent; }
  const canonical = path.join(fs.realpathSync(ancestor), ...suffix);
  const repository = fs.realpathSync(REPO_ROOT);
  if (canonical === repository || canonical.startsWith(repository + path.sep)) {
...
function atomicWriteFileSync(filePath, content, { mode = 0o600 } = {}) {
  ...
  try {
    ...
    const descriptor = fs.openSync(tmpPath, 'r');
    try { fs.fsyncSync(descriptor); } finally { fs.closeSync(descriptor); }
    fs.renameSync(tmpPath, filePath);
  } finally {
    try { fs.unlinkSync(tmpPath); } catch (err) { if (err.code !== 'ENOENT') { /* A failed cleanup must not obscure the original failure. */ } }
  }
}

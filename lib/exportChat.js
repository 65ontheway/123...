...
async function handleExportAwareChat(res, { upstreamMessages, selectedModel, maxTokens, apiKey, username, ownerId, appUrl, session }) {
  ...
  if (toolCall.function?.name !== 'export_file' || assistantMessage.tool_calls.length !== 1) return res.status(502).json({ ok: false, error: 'Unsupported export action.' });
  let args;
  try {
  ...
  try {
    ...
    const { id, expiresAt } = storeExport({ ...generated, ownerId });
  ...
  const explainRes = await providerFetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: baseHeaders,
    body: JSON.stringify({
      model: selectedModel,
  ...
  const extraChunk = exportMeta
    ? `data: ${JSON.stringify({ choices: [{ delta: { export: exportMeta } }] })}\n\n`
    : undefined;
  return pipeUpstreamStream(explainRes, res, { onUsage: (u) => addUsage(session, u), extraChunk });
}
